...
export async function handleGeneratePlan(request: Request, userId: string, deps: Deps): Promise<Response> {
  ...
  switch (outcome.kind) {
    case 'ok':
      return ok({ plan: outcome.plan, planId: outcome.planId, isFallback: outcome.isFallback });
...
function validateIntake(intake: IntakeResponses): string | null {
  if (typeof intake.goal !== 'string' || intake.goal.trim().length === 0) {
  ...
  if (typeof intake.daysPerWeek !== 'number' || intake.daysPerWeek < 1 || intake.daysPerWeek > 7) {
    return 'daysPerWeek must be between 1 and 7.';
  }
  if (typeof intake.weeklyKm !== 'number' || intake.weeklyKm < 0) {
    return 'weeklyKm must be a non-negative number.';
  }
  if (!Array.isArray(intake.injuries) || !intake.injuries.every((f) => INJURY_FLAGS.has(f))) {
    return 'injuries must be an array of the closed-set injury flags.';
  }
  ...
  if (intake.recentPerformance !== undefined) {
    ...
    if (!distance || typeof timeSec !== 'number' || timeSec <= 0) {
...
export async function handleGetPlan(userId: string, planId: string, deps: Deps): Promise<Response> {
  ...
  return ok({ plan: row.plan, planId: row.id, isFallback: row.isFallback });
}
