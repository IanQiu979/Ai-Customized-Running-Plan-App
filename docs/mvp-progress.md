...
- **Plan engine is fully wired, and every distance now gets its own training shape.** The pure
  ...
  distance-aware too (same date, later): the plumbing is built, but the real share/absolute
  numbers are captain-pending, not invented — see "Decided (2026-09-06, later)" and "Blocked".
...
| Marathon's long-run share/absolute ceilings are PROVISIONALLY non-binding for intermediate/advanced (`v22-distance-specific-plans`, `[key=marathon-longrun-share-cap]`) — engineering plumbing is built (`loadRules.ts`'s `longRunShareCap`/`maxSingleRunKm` both take `raceDistance`; `MARATHON_LONG_RUN_SHARE_CAP` is a single named constant, currently `Infinity`), but the real number is deliberately not invented | what a marathon long run should actually cap out at for intermediate/advanced runners — until set, marathon's long run is governed only by the 180-min time cap and the 10% spike guard | **Ian.** `report-source.md` line 66: "Maximum long-run share of weekly volume: no universal research-backed percentage exists"; line 102: this needs "Ian's judgment or licensed course material." Survey (50 km/wk, 16wk, intermediate) at 3/4/5/6 days/week, before → after: 25→28, 19→28, 16→28, 8→28 km — full table in `docs/change_log.md`'s 2026-09-06 (later) entry and `docs/reference/coaching/load-rules.md`. **Two decisions, not one:** while the ceilings are non-binding, marathon deload weeks are not bounded against their own reduced volume — marathon/16wk/50 km/4 days/intermediate ships a 16 km long run inside a 38 km deload week (42%, longer than weeks 1–3's 14/15/14 km), since only the 1.10× spike guard still applies. Setting the number fixes the **magnitude** only; the **structural** half — the long-run curves' fixed-position dips not realigning with `deloadEveryWeeks`' cadence once `interpolateCanonical` resamples them onto a non-canonical duration — is independent and survives it |
...
| Item | Decision |
|---|---|
| No marathon/half/10K training content — every distance stretched the 5K curve | **Fixed.** New per-distance weekly-load/long-run curves (`TEN_K_*`, `HALF_*`, `MARATHON_*` in `planTemplates.ts`), shaped from the research's already-resolved architecture (canonical durations, recovery-week positions) with peak long-run shares climbing from ~31% (5K) to ~50%+ (marathon). Regression: `planTemplates.distanceSpecific.test.ts`. |
...
| Long-run ceilings (`loadRules.ts`'s `longRunShareCap`/`MAX_SINGLE_RUN_KM`) are level-based, not distance-based | **Engineering built same day, real number still open — see "Decided (2026-09-06, later)" below and "Blocked" for the captain-owned number.** A 50 km/week, 16-week marathon plan now peaks at 28 km regardless of day count (previously 8-25 km depending on day count), governed by the time cap and spike guard until Ian sets the marathon-specific share/absolute numbers. |

## Decided (2026-09-06, later) — marathon long-run ceilings made distance-aware, PROVISIONAL/captain-pending

Firstmate engineering ruling on the `needs-decision` this task escalated (`[key=marathon-longrun-share-cap]`). Full account: `docs/change_log.md`'s 2026-09-06 (later) entry; full ruling text: `docs/reference/coaching/load-rules.md`'s 2026-09-06 (later) entry.
...
| Add a distance-aware dimension to the ceilings | **Built.** `longRunShareCap()`/`maxSingleRunKm()` (`loadRules.ts`) both take `raceDistance`; for marathon, both return `Infinity` (non-binding) for intermediate/advanced. `beginner` keeps its existing 14 km marathon ceiling — a deliberate, untouched safety floor for a first-timer's completion track. |
| What the actual marathon share/absolute numbers should be | **Not invented — held for Ian.** `report-source.md` explicitly says this needs his judgment. Until he rules, the 180-minute time cap and the (research-backed) 10% recent-longest-run spike guard govern marathon's long run instead. |
