...
| ~~`wrangler secret put GOOGLE_CLIENT_ID` / `GOOGLE_CLIENT_SECRET` (production)~~ | — | **Done 2026-08-09**, along with the production redirect URI. `sign-in/social` returns a real Google authorization URL and Google serves a live sign-in page against it. **Two open riders:** the client secret is only exercised at the token exchange, so one real sign-in from the app is still needed to prove it; and if the OAuth consent screen is in **Testing** status, only listed test users can complete sign-in (everyone else gets `access_denied` after entering their password). Whether the leaked secret was rotated before being deployed is unconfirmed — see the row below |
| **Rotate the Google client secret** (`v22-launch-audit-r1-decision-google-secret-rotation`) | nothing functionally, but the secret was pasted in plaintext into a chat pane twice during provisioning and must be treated as compromised | **Ian.** Approved but never confirmed performed. If the value deployed on 2026-08-09 is the original one, this is now a live production credential that has been exposed: Google Cloud Console → Credentials → the OAuth client → **+ ADD SECRET**, then delete the old one, then re-run `wrangler secret put GOOGLE_CLIENT_SECRET --env production`. Cheaper before launch than after |
