...
src/
  ...
  constants/
    ...
    pulseTrace.ts            # the pulse trace's OWN palette, timings and field heights (new
                              ...
                              #  have landed
    navigation-theme.ts      # bridges theme.ts's tokens into expo-router/react-navigation's `Theme`
...
- **React Navigation's own chrome is tokened too, not just the screens built on top of it.**
  ...
  `expo-router/react-navigation` expects (`background`→`surface.base`, `card`→`surface.raised`,
