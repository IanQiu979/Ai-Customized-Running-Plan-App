...
| Email | The Worker's provider-agnostic sender (`workers/src/lib/mail.ts`, `sendMail({ to, subject, text, html })`) with a Resend HTTP adapter, and a console/no-op adapter when unconfigured | Resend is switched on by `RESEND_API_KEY` + `MAIL_FROM` (captain-only secrets, `wrangler secret put … --env production`); with neither set nothing is sent, sign-up still works and the app says reset mail is unavailable. Runbook: [`docs/email-setup.md`](../docs/email-setup.md). **Superseded (2026-09-20, issue #94)** the original plan of Supabase built-in auth emails (v1) → dedicated SMTP (Resend/Postmark) before public launch. Still only verification / password-reset in v1 — no marketing/transactional email (no receipts until real payments in v2). |
