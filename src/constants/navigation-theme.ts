 ...
 * that untokened gray leak through (a light-mode push flashing `rgb(242, 242, 242)` against a
 * chalk `#F7F7F4` canvas is a visible seam). Every value below is derived from `Colors` in
 * `./theme.ts` — never a re-typed hex — so this can never drift from the rest of the system.
 *
 * `primary` maps to `text.primary`, not `Accent.hivis`: the brief reserves hivis for the single
 ...
 * this to `FontFamily.body` — the brief mandates Inter for UI chrome, and the stock block resolves
 * to system San Francisco / Roboto.
 */
